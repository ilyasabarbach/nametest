...
- Telegram mobile fallback sharing no longer auto-launches `tg://msg_url` or web share URLs when a prepared message is unavailable; it now stays in-app and shows a manual share sheet so Android Telegram no longer minimizes into the broken dark-webpage flow
...
4. verify Telegram `Share to chat` and `Share to story` behavior on actual Telegram clients, and if prepared chat share still has no `messageId`, check the returned `debugReason` first and confirm the live bot has inline mode enabled plus a real Main Mini App configured in BotFather
